...
continue_text = font.render("Click anywhere to play?", True, YELLOW, BLUE)
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            mouse_x = event.pos[0]
            mouse_y = event.pos[1]
            #if clown was clicked
            if clown_rect.collidepoint(mouse_x, mouse_y):
                hit_sound.play()
                score += 1
                clown_velocity += CLOWN_ACCERELATION
                #move clown in different direction when clicked
    ...
    if player_lives == 0:
        ...
        #pause game until player clickes
